...
| 피드 | 파일 | API/URL |
|------|------|---------|
| 증시·환율·원자재 | `pipeline/market_feed.js` | Yahoo Finance `query1.finance.yahoo.com/v8/finance/chart` |
| 국내 증시 | `collectors/domestic.js` | Yahoo `^KS11`·`^KQ11`, Naver `m.stock.naver.com/api/index` |
| 수급 | `collectors/domestic.js` | Naver `polling.finance.naver.com/api/realtime` |
| 해외 증시 | `collectors/overseas.js` | Yahoo `^DJI`, `^GSPC`, `^IXIC`, `^SOX`, `^N225`, `^GDAXI`, `^HSI` |
| 환율·금리 | `collectors/fx_rates.js` | Yahoo `USDKRW=X`, `DX-Y.NYB`, `^TNX`, `^IRX` |
| FOMC 확률 | `collectors/fx_rates.js` | ZQ 선물 `ZQK26`, `ZQM26`, `ZQU26` (CME via Yahoo) |
| 원자재 | `collectors/commodities.js` | Yahoo `GC=F`, `SI=F`, `CL=F`, `HG=F`, `ALI=F` |
| 국내 금 | `collectors/commodities.js` | Naver `finance.naver.com/marketindex/goldDailyQuote.nhn` (EUC-KR) |
| 원시 뉴스 | `collectors/news.js` | Naver Search API `openapi.naver.com/v1/search/news` |
| 공시 | `pipeline/dart_feed.js` | OpenDART `opendart.fss.or.kr/api` |
| 코인 | `pipeline/crypto_feed.js` | CoinGecko `api.coingecko.com/api/v3` |
